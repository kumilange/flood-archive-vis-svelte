<script lang="ts">
  ...
  const timeRange: RangeValues = getTimeRange(data.features);
  let rangeValues = $state<RangeValues>(timeRange);
  const layers = $derived([createFloodLayer(data, rangeValues)]);
